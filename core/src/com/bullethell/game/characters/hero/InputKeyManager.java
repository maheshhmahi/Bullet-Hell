...
public class InputKeyManager {
    ...
    private InputKeyManager(Hero hero){
        this.hero = hero;
        shootingPace = 100;
        lastFireTime = TimeUtils.millis();
    }

    public static InputKeyManager getInstance(Hero hero)
    {
        if (_instance == null)
            _instance = new InputKeyManager(hero);
        return _instance;
    }
    ...
    public void executeInputCommand(float delta) {
        if(Gdx.input.isKeyPressed(leftKey))
            hero.setPosition(hero.getPos().x - hero.getCurrentSpeed()*delta, hero.getPos().y);

        if(Gdx.input.isKeyPressed(rightKey))
            hero.setPosition(hero.getPos().x + hero.getCurrentSpeed()*delta, hero.getPos().y);

        if(Gdx.input.isKeyPressed(upKey))
            hero.setPosition(hero.getPos().x,hero.getPos().y + hero.getCurrentSpeed()*delta);
...
//        }
        ...
        if(Gdx.input.isKeyPressed(slowSpeedKey))
            hero.setCurrentSpeed((int)(150));
        else
            hero.setCurrentSpeed((int)(250));

        if(Gdx.input.isKeyPressed(normalModeKey))
            hero.setCurrentSpeed(hero.getNormalSpeed());

        if(Gdx.input.isKeyPressed(slowModeKey))
            hero.setCurrentSpeed(hero.getSlowSpeed());
    }
}
